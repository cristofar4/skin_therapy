        ...
        <AnimatePresence mode="popLayout">
          {list.map((t, i) => (
            <motion.button
              ...
              onClick={() => setActive(t)}
              className="group relative flex flex-col overflow-hidden rounded-[2rem] bg-card text-left shadow-soft"
            >
              <div className="relative aspect-[4/5] overflow-hidden">
                ...
                <span className="absolute left-5 top-5 rounded-full bg-cream/90 px-4 py-1.5 text-[0.65rem] uppercase tracking-[0.14em] text-charcoal">
                  {t.category}
                </span>
                <div className="absolute inset-x-5 bottom-5 flex items-center justify-between text-cream">
              ...
              <div className="p-6">
                <h3 className="font-serif text-2xl text-charcoal">{t.name}</h3>
                <p className="mt-2 text-sm text-charcoal-light">{t.tagline}</p>
...
function TreatmentDialog({ treatment, onClose }: { treatment: Treatment; onClose: () => void }) {
  return (
    <motion.div
      ...
      exit={{ opacity: 0 }}
      onClick={onClose}
      className="fixed inset-0 z-[300] flex items-end justify-center bg-charcoal/60 p-0 backdrop-blur-sm md:items-center md:p-6"
    >
      <motion.div
        ...
        className="relative grid max-h-[92vh] w-full max-w-4xl overflow-hidden rounded-t-[2rem] bg-cream md:max-h-[88vh] md:grid-cols-2 md:rounded-[2rem]"
      >
        <button
          onClick={onClose}
          aria-label="Close"
          className="absolute right-4 top-4 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-cream/90 text-charcoal shadow-soft md:bg-white/20 md:text-cream"
        >
          <X className="h-5 w-5" />
        </button>
        ...
        <div className="overflow-y-auto p-8 md:p-10">
          ...
          <h3 className="mt-4 font-serif text-4xl text-charcoal">{treatment.name}</h3>
          <p className="mt-3 text-pretty leading-relaxed text-charcoal-light">{treatment.overview}</p>

          <h4 className="mt-7 flex items-center gap-2 text-xs uppercase tracking-[0.16em] text-charcoal-soft">
          ...
          <div className="mt-7 grid grid-cols-2 gap-4">
            <div className="rounded-2xl bg-beige-100 p-4">
              <p className="text-[0.65rem] uppercase tracking-[0.14em] text-charcoal-light">Duration</p>
              <p className="mt-1 font-serif text-xl text-charcoal">{treatment.duration}</p>
            </div>
            <div className="rounded-2xl bg-beige-100 p-4">
              <p className="text-[0.65rem] uppercase tracking-[0.14em] text-charcoal-light">Recovery</p>
              <p className="mt-1 text-sm text-charcoal">{treatment.recovery}</p>
            </div>
          </div>
