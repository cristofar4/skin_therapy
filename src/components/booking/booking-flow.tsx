...
export function BookingFlow() {
  ...
  return (
    <div className="mx-auto max-w-4xl">
              ...
              <div
                className={`flex h-10 w-10 items-center justify-center rounded-full border text-sm transition-colors duration-500 ${
                  i < step
                    ...
                    : i === step
                      ? 'border-ivory bg-onyx text-ivory'
                      : 'border-ivory/20 text-ivory-dim'
                }`}
              >
                {i < step ? <Check className="h-4 w-4" /> : i + 1}
              </div>
              <span className="hidden text-[0.6rem] uppercase tracking-[0.12em] text-ivory-dim md:block">
                {label}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div className="mx-2 h-px flex-1 bg-onyx/15">
                <div
      ...
      <div className="min-h-[440px] rounded-[2rem] bg-surface p-6 shadow-soft md:p-10">
                ...
                {treatments.map((t) => (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <p className="truncate font-serif text-lg text-ivory">{t.name}</p>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-2">
                <button
                  ...
                  className={`rounded-2xl border p-5 text-left transition-all ${
                    therapist === 'Any Available' ? 'border-champagne bg-champagne/10' : 'border-ivory/10 hover:border-ivory/30'
                  }`}
                >
                  <div className="flex h-14 w-14 items-center justify-center rounded-full bg-surface-2 font-serif text-xl text-champagne-deep">
                    St
                  </div>
                  <p className="mt-4 font-serif text-xl text-ivory">Any Available</p>
                  <p className="text-sm text-ivory-dim">We will match you with the perfect expert</p>
                </button>
                {therapists.map((t) => (
                  <button
                    key={t.name}
                    ...
                    className={`flex items-center gap-4 rounded-2xl border p-5 text-left transition-all ${
                      therapist === t.name ? 'border-champagne bg-champagne/10' : 'border-ivory/10 hover:border-ivory/30'
                    }`}
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-ivory-dim">{t.specialty}</p>
                    </div>
                  </button>
                ))}
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center rounded-2xl border p-4 transition-all ${
                        date === key ? 'border-champagne bg-champagne/10' : 'border-ivory/10 hover:border-ivory/30'
                      }`}
                    >
                      <span className="text-[0.65rem] uppercase tracking-[0.12em] text-ivory-dim">
                        {weekday}
                      </span>
                      <span className="mt-1 font-serif text-2xl text-ivory">{d.getDate()}</span>
                      <span className="text-[0.6rem] text-ivory-dim">
                ...
                {times.map((t) => (
                  <button
                    ...
                    className={`rounded-2xl border py-5 font-serif text-2xl transition-all ${
                      time === t ? 'border-champagne bg-champagne/10 text-champagne-deep' : 'border-ivory/10 text-ivory hover:border-ivory/30'
                    }`}
                  >
                    {t}
                  </button>
          ...
          {step === 4 && (
            <StepShell key="s4" icon={<User />} title="Your details">
              <div className="grid gap-5">
                ...
                <div className="rounded-2xl bg-surface p-5 text-sm text-ivory-soft">
                  <p className="font-medium text-ivory">Booking summary</p>
          ...
          {step === 5 && (
            ...
            >
              <motion.div
                ...
                className="flex h-20 w-20 items-center justify-center rounded-full bg-champagne text-ivory"
              >
                <PartyPopper className="h-9 w-9" />
              </motion.div>
              <h3 className="mt-6 font-serif text-4xl text-ivory">Your ritual is reserved</h3>
              ...
              <div className="mt-8 w-full max-w-md rounded-2xl border border-ivory/10 bg-ink p-6 text-left">
                <div className="flex items-center justify-between border-b border-ivory/10 pb-4">
                  <span className="font-serif text-xl text-ivory">Confirmation</span>
                  <span className="rounded-full bg-onyx px-3 py-1 text-xs text-ivory">{ref}</span>
                </div>
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <Link
                  ...
                  target="_blank"
                  className="flex h-12 items-center justify-center rounded-full bg-champagne px-8 text-xs uppercase tracking-[0.16em] text-ivory"
                >
                  Confirm on WhatsApp
                </Link>
                <Link
                  href="/"
                  className="flex h-12 items-center justify-center rounded-full border border-ivory/20 px-8 text-xs uppercase tracking-[0.16em] text-ivory"
                >
                  Return Home
                </Link>
              </div>
      ...
      {step < 5 && (
        <div className="mt-8 flex items-center justify-between">
          <button
            onClick={() => go(-1)}
            disabled={step === 0}
            className="inline-flex items-center gap-2 text-xs uppercase tracking-[0.16em] text-ivory disabled:opacity-30"
          >
          ...
          <button
            onClick={() => go(1)}
            disabled={!canNext}
            className="inline-flex h-12 items-center gap-2 rounded-full bg-onyx px-8 text-xs uppercase tracking-[0.16em] text-ivory transition-opacity disabled:opacity-30"
          >
