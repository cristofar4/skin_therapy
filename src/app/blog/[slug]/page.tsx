    ...
    <article>
      <section className="relative flex min-h-[68vh] items-end overflow-hidden pb-16 pt-40">
        ...
        <div className="container-luxe relative z-10">
          <Link
            href="/blog"
            className="inline-flex items-center gap-2 text-xs uppercase tracking-[0.14em] text-cream/70 hover:text-champagne-light"
          >
            <ArrowLeft className="h-4 w-4" /> Back to Journal
          </Link>
          <span className="eyebrow mt-6 text-champagne-light">{post.category}</span>
          <SplitText as="h1" className="mt-4 max-w-4xl text-4xl text-cream md:text-6xl">
            {post.title}
          </SplitText>
          <div className="mt-6 flex items-center gap-4 text-xs uppercase tracking-[0.14em] text-cream/70">
            <span>{post.date}</span>
      ...
      <section className="container-luxe py-20">
        <div className="mx-auto max-w-2xl">
          ...
          <div className="mt-10 space-y-6 text-pretty text-lg leading-relaxed text-charcoal-soft">
          ...
          <div className="mt-14 rounded-[1.75rem] bg-beige-100 p-8 text-center">
            <p className="font-serif text-2xl text-charcoal">Ready to experience it yourself?</p>
            <p className="mt-2 text-sm text-charcoal-light">
              Book a consultation and let our therapists craft a ritual for your skin.
            </p>
            <Link
              href="/book"
              className="mt-6 inline-flex h-12 items-center justify-center gap-2 rounded-full bg-charcoal px-8 text-xs uppercase tracking-[0.16em] text-cream"
            >
              Book Appointment
            </Link>
          </div>
      ...
      <section className="container-luxe pb-24">
        <h2 className="mb-10 font-serif text-3xl text-charcoal">Continue reading</h2>
        <div className="grid gap-8 md:grid-cols-3">
          {more.map((p) => (
            <Link key={p.slug} href={`/blog/${p.slug}`} className="group flex flex-col">
              ...
              <h3 className="mt-5 font-serif text-xl text-charcoal transition-colors group-hover:text-champagne-deep">
                {p.title}
              </h3>
